...
class FaqApi {
  ...
  Future<NetworkResponse> getFaqList() async {
    final res = await _getRequest(
      path: Endpoint.getFaq,
    );
    return res;
  }

  Future<NetworkResponse> getChatBox(parenId) async {
    final res = await _getRequest(
        path: Endpoint.getChatbox, params: {"next_step_id": parenId});
    return res;
  }
}
